...
class SubcircuitSynthesiser :
  ...
  def bottomUpReduction(self, to_replace, nof_gate_inputs, log_enc) :
    if log_enc :
      self.log_encodings = True
      self.encoding_log_dir = log_enc[0]
    ...
    while True :
      ...
      current_realisable, subcir = self._checkEncoding(encoder, to_replace, circuit_size, nof_gate_inputs, self.config)
  ...
  def _runSolverAndGetAssignment(self, input, timeout = None) :
    if self.config.qbf_solver == utils.Configuration.QBFSolver.miniQU :
      ...
      output_pattern = r"\nv\s*(.*)\n*"
    elif self.config.qbf_solver == utils.Configuration.QBFSolver.qute :
      solver_cmd = [qute_path, "--partial-certificate", input]
      output_pattern = r"SAT\n(.*)0\n"
    elif self.config.qbf_solver == utils.Configuration.QBFSolver.caqe :
